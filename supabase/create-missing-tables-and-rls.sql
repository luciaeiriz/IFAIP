-- ============================================
-- CREATE MISSING TABLES AND SETUP RLS
-- Assumes courses table already exists
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '⚠️ courses.id type: %', current_type;
    END IF;
END $$;

-- Step 3: Create leads table (only if it doesn't exist)
...
-- Step 6: Create function to update updated_at timestamp (if it doesn't exist)
...
-- Step 7: Create trigger for courses updated_at (drop first if exists)
...
-- Step 13: Create indexes (if they don't exist)
...
-- Test: Should return course counts
