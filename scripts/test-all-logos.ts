...
// Generate logo URL from provider name (same logic as in components)
function getLogoUrl(provider: string | null): string | null {
  ...
  const cleanedProvider = normalizedProvider
    .replace(/\b(university|univ|college|school|institute|inc|llc|corp|corporation|ltd|limited)\b/g, '')
    .trim()
    .replace(/\s+/g, '')
  
  const commonTlds = ['.com', '.org', '.edu', '.net', '.io']
  for (const tld of commonTlds) {
    const potentialDomain = cleanedProvider + tld
    return `/api/logo/${encodeURIComponent(potentialDomain)}`
  }

  return `/api/logo/${encodeURIComponent(normalizedProvider.replace(/\s+/g, ''))}`
}
